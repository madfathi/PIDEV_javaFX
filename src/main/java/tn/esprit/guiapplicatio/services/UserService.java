...
//import tn.esprit.guiapplicatio.models.c;
...
public class UserService implements IServices<User> {
    ...
    public void envoyerCode(String email, String code) {
        ...
        try {
            ...
            message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(email)); // Adresse e-mail de destination
            message.setSubject("Compte cree avec succes"); // Sujet de l'e-mail
            message.setText("Votre compte a ete cree avec succes vous trouvez ci dessous votre code promo que vous pouvez utulisez lors de votre premiere commande : " + code); // Corps de l'e-mail avec le code de récupération

            // Envoi du message
            Transport.send(message);

            System.out.println("Code promo envoyé avec succès à " + email);
